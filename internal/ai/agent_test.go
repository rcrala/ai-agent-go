...
func TestEvaluateWithBackoff(t *testing.T) {
	...
	t.Run("success on first try", func(t *testing.T) {
		...
		result, err := evaluateWithBackoff(ctx, mock, "test.go", "code", cfg)
		if err != nil {
	...
	t.Run("success after retries on 429", func(t *testing.T) {
	...
	t.Run("exhaust retries on 429", func(t *testing.T) {
...
func TestCircuitBreaker(t *testing.T) {
	...
	t.Run("circuit closes after wait period", func(t *testing.T) {
		cb := &CircuitBreakerState{}
		maxFailures := 2
...
// TestEvaluationMetrics validates metrics tracking
func TestEvaluationMetrics(t *testing.T) {
	ctx := context.Background()

	t.Run("tracks successful evaluation", func(t *testing.T) {
		mock := &mockEvaluator{failUntil: 0}
		cfg := AIAgentConfig{MaxRetries: 0}
		metrics := &EvaluationMetrics{}

		_, err := evaluateWithBackoffAndMetrics(ctx, mock, "test.go", "code", cfg, metrics, nil)
		if err != nil {
	...
	t.Run("tracks failures and retries", func(t *testing.T) {
		mock := &mockEvaluator{failUntil: 2, returnCode: 429}
		cfg := AIAgentConfig{MaxRetries: 3, BackoffInitialMs: 10}
		metrics := &EvaluationMetrics{}
	...
	t.Run("tracks circuit breaker trips", func(t *testing.T) {
		mock := &mockEvaluator{failUntil: 10, returnCode: 429}
...
func TestBackoffJitter(t *testing.T) {
	...
	t.Run("jitter adds variance to backoff", func(t *testing.T) {
		...
		for i := 0; i < 5; i++ {
			...
			start := time.Now()
			_, _ = evaluateWithBackoffAndMetrics(ctx, mock, "test.go", "code", cfg, nil, nil)
